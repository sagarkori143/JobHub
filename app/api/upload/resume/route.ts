...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      ...
      return NextResponse.json({ error: "Google Generative AI API key is missing. Set GOOGLE_GENERATIVE_AI_API_KEY in your environment." }, { status: 500 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: `Error processing resume scoring request: ${e instanceof Error ? e.message : String(e)}` }, { status: 500 });
    }
