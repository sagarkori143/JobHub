...
export default function InterviewingJobsPage() {
  ...
  useEffect(() => {
    const fetchInterviewingJobs = async () => {
      setLoading(true)
      setError(null)
      try {
        const allJobs = await jobDataService.getJobs()
        const interviewing = allJobs.filter((job) => job.status === "Interviewing")
        setJobs(interviewing)
      } catch (err) {
        console.error("Failed to fetch interviewing jobs:", err)
        setError("Failed to load interviewing jobs. Please try again later.")
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Interviewing Jobs</h1>
      <Card className="mb-6">
        <CardHeader>
        ...
        <CardContent>
          <div className="relative mb-4">
            ...
            <Input
              type="text"
              placeholder="Search interviewing jobs..."
              value={searchTerm}
          ...
          <div className="grid gap-4">
            {paginatedJobs.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground">No interviewing jobs found.</p>
            )}
          </div>
          {totalPages > 1 && (
